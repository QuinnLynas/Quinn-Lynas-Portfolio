...
data <- read_csv("seawatch_noaa.csv") |>
  ...
  output$plot <- renderPlot({
    if(input$plot == "Polar Degrees"){
      ...
      filteredData() |>
        ...
        ggplot() +
        aes(x = factor(comp_dir, levels = c("N","NNE","NE", "ENE", "E", "ESE", "SE", "SSE",   #label compass directions
                                            "S", "SSW", "SW", "WSW", "W", "WNW", "NW", "NNW")), y = count, colour = pres_h_pa, shape = common_name) +
        geom_jitter(position = position_jitter(height = 0.3, width = 0.2)) +
        scale_color_viridis_c(option = "viridis", direction = 1) +
        theme_classic() + 
        scale_y_continuous(trans = "sqrt") +
      ...
      filteredData() |> 
        filter(!is.na(comp_dir)) |>
        filter(!is.na(pres_h_pa)) |>
        ggplot() +
        aes(x = factor(comp_dir, levels = c("N","NNE","NE", "ENE", "E", "ESE", "SE", "SSE",   #label compass directions
                                            "S", "SSW", "SW", "WSW", "W", "WNW", "NW", "NNW")), y = count, fill = factor(cut(pres_h_pa, breaks = 6))) +
        geom_col() +
